...
1. Test and play within the interactive R console then copy code into a .R file to run later. (I do NOT recommend using RStudio in this way.)
2. Start writing in an .R file and use RStudio’s shortcut keys for the Run command to push the current line, selected lines, or modified lines to the interactive R console. (This is a great way to use R/RStudio because all of your code is saved and you will be able to run the file later on.)

# The R interactive console
...
Once we've downloaded a package, we can use it. R does not keep all packages in its working memory, however--that would slow R down. Instead, we need to "load" the package(s) we are interested in using into our R session. We must load these package(s) into our R session *each time we open R (or RStudio)*.

```{r}
library(dplyr)
```

You'll notice that when we call `dplyr` into our environment, R spits out some information: a soft warning, saying that this package was built under a different version of R; that R is attaching the `dplyr` package; and that a few objects are masked from different packages. These masked objects are functions that conflict with functions built into `dplyr`. 

*Remember* : to call a function from a particular package, we use the `::` (e.g., dplyr::filter).

# Project Management with RStudio

RStudio's project management functionality is INTUITIVE and POWERFUL.
...
So... what is a dataframe? Think of it like an excel spreadsheet, but housed in R in a very specific way: Where all columns must be of the same type (think... vectors). Dataframes can be massive (think thousands of columns of genetic data), or relatively small, think a dozen columns of information relevant to a research study. Today we are going to work with an R dataset called `gapminder`.

```{r}
# We can either download the `gapminder` dataset off github, or:
...
# We can pull n the data from our data folder
gapminder <- read.csv("data/gapminder_data.csv")
```

Let's check out the data. Data exploration is one of the most important parts of working in R--and it is the first thing you should always do when looking at new data.
```{r}
...
In a factor, each unique value represents what is called a **level**. In our case, the factor “continent” has 5 levels: “Africa”, “Americas”, “Asia”, “Europe” and “Oceania”. R will only accept values that match one of these levels. If you add a new value, it will coerce the unknown NEW level to "`NA`".
...
Remember the columns in data frames are lists, so similar rules apply. However, they are also two dimensional objects:
...
Luckily, the `dplyr` package provides a number of very useful functions for manipulating dataframes in a way that will reduce the above repetition, while reducing the probability of making errors. As an added bonus, you might even find the `dplyr` grammar easier to read.
...
First we summon the gapminder data frame and pass it on, using the pipe symbol %>%, to the next step, which is the `select()` function. In this case we don’t specify which data object we use in the `select()` function since it gets that information from the previous pipe. Fun Fact: You may have encountered pipes before in the shell. In R, a pipe symbol is %>% while in the shell it is | but the concept is the same.
...
The power of dplyr is that you don't have to do repetitive tasks that might produce bugs in your code; thus far we haven not been using this functionality... we would have to make a new dataframe for each country... this is where `group_by()` steps in!
...
The data is now in a grouped_df, not the same as the data.frame we originally saw. The grouped_df is essentially a list where each item in the list is its own dataframe that only contains the rows that correspond to a particular value of continent.

Using `group_by()` in this way can seem pretty uneventful... that is because `group_by()` simply holds onto the data. You must pass other functions to it in order to really see its power. We'll use `summarise()` which allows us to create new variables by using functions that repeat for each of the grouped datasets.
...
You can also chain together several summary operations; in this case calculating the minimum, maximum, mean and standard error of each continent’s per-country life-expectancy:
...
# Last Step!! Data Writing/Saving

One of the key elements of "good" data practices is NEVER to over-write raw data. Raw-data is sacred, and needs to exist in your project in its original format. Manipulated data is important to save as well. You might place these data in folders called `analysis-data` or `data-out`; regardless of what you call this folder, you will always save manipulated data to this folder!
